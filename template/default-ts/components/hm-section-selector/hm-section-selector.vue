<template>
  <view class="hm-subsection" :style="[subsectionStyle]">
    <view
      class="hm-item hm-clamp-1"
      :style="[itemStyle(index)]"
      @tap="click(index)"
      :class="[noBorderRight(index), 'hm-item-' + index]"
      v-for="(item, index) in listInfo"
      :key="index"
    >
      <view :style="[textStyle(index)]" class="hm-item-text hm-clamp-1">{{ item.name }}</view>
    </view>
    <view class="hm-item-bg" :style="[itemBarStyle]"></view>
  </view>
</template>

<script>
export default {
  name: 'hm-section-selector',
  props: {
    // tab的数据
    list: {
      type: Array,
  ...
  computed: {
    ...
    noBorderRight() {
      return index => {
        ...
        let classs = ''
        // 不显示右边的边框
        if (index < this.list.length - 1) classs += ' hm-none-border-right'
        // 显示整个组件的左右边圆角
        if (index === 0) classs += ' hm-item-first'
        if (index === this.list.length - 1) classs += ' hm-item-last'
        return classs
      }
    },
    // 文字的样式
  ...
  methods: {
    ...
    getTabsInfo() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        view.select('.hm-item-' + i).boundingClientRect()
      }
      view.exec(res => {
        if (!res.length) {
          setTimeout(() => {
